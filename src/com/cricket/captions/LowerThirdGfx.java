...
public class LowerThirdGfx 
{
	...
	public Player outPlayer,inPlayer;
	...
	public ForeignLanguageData dichowOut;
	...
	public void checkForImpactPlayer(String whatToProcess, int whichSide, int playerId, MatchAllData matchAllData) {
		...
		case Constants.APLT20:
			...
			case "F6": case "Control_F6": case "Shift_F6": case "F5": case "F9": case "Control_F5": case "Control_F9": case "Shift_F5": case "Shift_F9":
				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$Text$Side" + whichSide 
						+ "$img_Text1$English$Name$select_Impact*FUNCTION*Omo*vis_con SET 0\0", print_writers);
				impactType = CricketFunctions.checkBatAndBallImpactInOutPlayer(matchAllData.getEventFile().getEvents(), playerId);
				if (!impactType.isEmpty()) {
					switch(impactType) {
					case "IMP_IN": case "CON_IN":
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$Text$Side" + whichSide 
								+ "$img_Text1$English$Name$select_Impact*FUNCTION*Omo*vis_con SET 2\0", print_writers);
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$Text$Side" + whichSide 
								+ "$img_Text1$English$Name$ImpactAll$Impact$txt_Name*GEOM*TEXT SET " + impactType.replace("_IN", "") + "\0", print_writers);
						break;
					case "IMP_OUT": case "CON_OUT":
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$Text$Side" + whichSide 
								+ "$img_Text1$English$Name$select_Impact*FUNCTION*Omo*vis_con SET 1\0", print_writers);
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$Text$Side" + whichSide 
								+ "$img_Text1$English$Name$ImpactAll$Sub$txt_Name*GEOM*TEXT SET " + (impactType.equalsIgnoreCase("IMP_OUT") 
										? "SUB" : "CON") + "\0", print_writers);
						break;
					}
				}
				break;
			case "F8": case "Alt_F8":
				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper_Center$TopLine$img_Text1$Side" + whichSide 
						+ "$English$Name$select_Impact*FUNCTION*Omo*vis_con SET 0\0", print_writers);
				impactType = CricketFunctions.checkBatAndBallImpactInOutPlayer(matchAllData.getEventFile().getEvents(), playerId);
				if (!impactType.isEmpty()) {
					switch(impactType) {
					case "IMP_IN": case "CON_IN":
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper_Center$TopLine$img_Text1$Side" + whichSide 
								+ "$English$Name$select_Impact*FUNCTION*Omo*vis_con SET 2\0", print_writers);
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper_Center$TopLine$img_Text1$Side" + whichSide 
								+ "$English$Name$ImpactAll$Impact$txt_Name*GEOM*TEXT SET " + impactType.replace("_IN", "") + "\0", print_writers);
						break;
					case "IMP_OUT": case "CON_OUT":
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper_Center$TopLine$img_Text1$Side" + whichSide 
								+ "$English$Name$select_Impact*FUNCTION*Omo*vis_con SET 1\0", print_writers);
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper_Center$TopLine$img_Text1$Side" + whichSide 
								+ "$English$Name$ImpactAll$Sub$txt_Name*GEOM*TEXT SET " + (impactType.equalsIgnoreCase("IMP_OUT") ? "SUB" : "CON") + "\0", print_writers);
						break;
					}
				}
				break;
...
//		Team team = Teams.stream().filter(tm -> tm.getTeamId() == outPlayer.getTeamId()).findAny().orElse(null);
	...
	public String populateBowlingStyle(String whatToProcess,int WhichSide,MatchAllData matchAllData) throws InterruptedException, JsonMappingException, JsonProcessingException
	{
		...
		checkForImpactPlayer(whatToProcess, WhichSide, player.getPlayerId(), matchAllData);
...
//				setPositionOfLT(lowerThird.getNumberOfSubLines(), WhichSide, 4,print_writers, config);
	...
	public String populateLTNameSuperPlayer(String whatToProcess, int whichSide, MatchAllData matchAllData) throws InterruptedException, JsonMappingException, JsonProcessingException {
		...
		case Constants.TG20: case Constants.APLT20:
			lowerThird = new LowerThird("", player.getFirstname(), surName, player.getFull_name(),team.getTeamName1(), 
					"", 1, "",team.getTeamBadge(),null,null,new String[]{WhichProfile},null,null);
			break;
		}
...
//				setPositionOfLT(whatToProcess,whichSide,config,lowerThird.getNumberOfSubLines());
	...
	public String PopulateLofPlayerProfile(String whatToProcess, int WhichSide, MatchAllData matchAllData) 
			throws JsonMappingException, JsonProcessingException, InterruptedException
		{
			if(!whatToProcess.contains(",") && whatToProcess.split(",").length >= 4) {
				return "PopulateL3rdPlayerProfile: error occured in what to process";
			...
			String Data = "",hundred = "",fifty = "",strikeRate = "", thirty = "",
				batAverage = "",economy = "",best = "",runs = "",short_name = "",against = "",venue = "";
			...
			if(whatToProcess.split(",")[0].equalsIgnoreCase("Control_3")) {
				...
				against = (stat.getBestScoreAgainst() != null ? (stat.getBestScoreAgainst().contains("v ")?stat.getBestScoreAgainst():"v "+stat.getBestScoreAgainst()) : "");
				venue = (stat.getBestScoreVenue() != null ? stat.getBestScoreVenue() : "");
				...
				case Constants.ACC:
					...
					}else if(WhichProfile.equalsIgnoreCase("ACC_CAREER")) {
						short_name =  "ACC T20 CAREER";
					}else {
						short_name = "T20 CAREER";
					}
					switch (config.getBroadcaster().toUpperCase()) {
					...
					case Constants.ACC:
						...
						if(WhichProfile.equalsIgnoreCase("DT20") || WhichProfile.equalsIgnoreCase("IT20") || 
								WhichProfile.equalsIgnoreCase("ACC_CAREER")) {
							lowerThird = new LowerThird(CricketFunctions.getbattingstyle(player.getBattingStyle(),CricketUtil.FULL, true, false).toUpperCase(), player.getFirstname(), surName,short_name,
									whatToProcess.split(",")[4], player.getPhoto(), 2,team.getTeamName4(),team.getTeamBadge(),
									new String[]{"MATCHES", "RUNS", "50s / 100s", "STRIKE RATE","BEST"},
									new String[]{String.valueOf(stat.getMatches()), runs , fifty + " / " + hundred,strikeRate,best},new String[] {against,venue},new String[] {"WITHOUT"},
									new String[] {"-165","-90","-8","72","157"});
			...
			else if(whatToProcess.split(",")[0].equalsIgnoreCase("Control_8")) {
				...
				against = (stat.getBestFiguresAgainst() != null ? (stat.getBestFiguresAgainst().contains("v ")?stat.getBestFiguresAgainst():"v "+stat.getBestFiguresAgainst()) : "");
				venue = (stat.getBestFiguresVenue() != null ? stat.getBestFiguresVenue() : "");
				...
				case Constants.ACC:
					...
					lowerThird = new LowerThird(CricketFunctions.getbowlingstyle(player.getBowlingStyle()).toUpperCase(), 
							player.getFirstname(), surName,short_name, whatToProcess.split(",")[4], player.getPhoto(), 2,team.getTeamName4(),team.getTeamBadge(),new String[]{"MATCHES", "WICKETS", "3WI / 5WI", "ECONOMY","BEST"},
							...
							economy,stat.getBestFigures()},new String[] {against,venue},null,new String[] {"-165","-77","11","83","163"});
					break;
...
//				setPositionOfLT(whatToProcess,WhichSide,config,lowerThird.getNumberOfSubLines());
		...
		if(whatToProcess.split(",")[0].equalsIgnoreCase("Control_s")) {
			...
			case Constants.TG20: case Constants.APLT20:
				lowerThird = new LowerThird(CricketFunctions.getbowlingstyle(player.getBowlingStyle()).toUpperCase(), tournament.getPlayer().getFirstname(), 
						surName,"THIS TOURNAMENT", "", "", 2,"",teamName,new String[]{"MATCHES", "WICKETS", "3WI", "ECONOMY", "BEST"},
	...
	public String populateTeamSummary(String whatToProcess,int WhichSide,MatchAllData matchAllData) throws InterruptedException, JsonMappingException, JsonProcessingException
	{
		...
		case Constants.TRI_SERIES:  case Constants.MT20: case Constants.BAN_AFG_SERIES: case Constants.ACC:
			lowerThird = new LowerThird(inning.getBatting_team().getTeamName1(), "", "","", String.valueOf(inning.getTotalRuns()) + "-" + String.valueOf(inning.getTotalWickets()), 
					...
					String.valueOf(inning.getTotalSixes())},null,null,new String[] {"-174","-110","-38","37","113","178"});
			break;
		case Constants.TG20: case Constants.APLT20:
			lowerThird = new LowerThird(inning.getBatting_team().getTeamName1(), "", "","", String.valueOf(inning.getTotalRuns()) + "-" + String.valueOf(inning.getTotalWickets()), 
					CricketFunctions.OverBalls(inning.getTotalOvers(), inning.getTotalBalls()), 2, "", inning.getBatting_team().getTeamBadge(),
	...
	public String populatePhaseWiseRunRate(String whatToProcess, int whichSide, MatchAllData matchAllData) throws InterruptedException, JsonMappingException, JsonProcessingException {
		...
		switch (config.getBroadcaster().toUpperCase()) {
		case Constants.TG20: case Constants.APLT20:
			lowerThird = new LowerThird("", inning.getBatting_team().getTeamName1(), inning.getBatting_team().getTeamName3(),"",CricketFunctions.getTeamScore(inning, "-", false), 
					...
					new String[]{"1 - 6", "7 - 15", "16 - "+ titl},new String[]{(PP1.equalsIgnoreCase("-") ? "-" : CricketFunctions.generateRunRate(Integer.valueOf(PP1.split("-")[0]), 
							CricketFunctions.getPowerplayOvers("1-6", inning.getTotalOvers()), 0, 2, matchAllData)),
							(PP2.equalsIgnoreCase("-") ? "-" :CricketFunctions.generateRunRate(Integer.valueOf(PP2.split("-")[0]), 
									CricketFunctions.getPowerplayOvers("7-15", inning.getTotalOvers()), 0, 2, matchAllData)),(PP3.equalsIgnoreCase("-") ? "-" : CricketFunctions.generateRunRate(Integer.valueOf(PP3.split("-")[0]), 
											CricketFunctions.getPowerplayOvers("16-20", inning.getTotalOvers()), 0, 2, matchAllData))},
					new String[] {"OVERS", "RUN RATE"},null,new String[]{"-97","31","171"});
			break;
		}
...
//						}
	...
	public String populatePhaseComp(String whatToProcess, int whichSide, MatchAllData matchAllData) throws InterruptedException, JsonMappingException, JsonProcessingException {
		...
		if(matchAllData.getSetup().getMatchType().equalsIgnoreCase(CricketUtil.ODI) ||
			...
			lowerThird = new LowerThird("", "COMPARISON", "","","", "",3,"","TLogo",
					...
					sevenToFifteenRuns1 + "," + sevenToFifteenRuns2 + "-" + sevenToFifteenWkt1 + "," + sevenToFifteenWkt2,sixteenToTweentyRuns1 + "," + 
					sixteenToTweentyRuns2 + "-" + sixteenToTweentyWkt1 + "," + sixteenToTweentyWkt2},
					new String[] {"OVERS", "SCORE"},null,new String[]{"-85","31","171"});
		}
	...
	public String InfobarManhattan(List<PrintWriter> print_writers, MatchAllData matchAllData,int WhichInning) {
		...
		case Constants.TG20: case Constants.APLT20:
			CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_Manhattan$TopLine$LogoAll$img_Logos*TEXTURE*IMAGE SET " 
					+ Constants.TG20_LOGO + inning.getBatting_team().getTeamBadge() + "\0", print_writers);
			
			CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$LT_Manhattan$BarAll$Runs_Axis$Title$select_Language"
			...
			CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$LT_Manhattan$BarAll$Runs_Axis$Title$English$txt_Tittle"
					+ "*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
			
			CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$LT_Manhattan$SublineGrp$Overs_Axis$Overs_Data$Title$select_Language"
					+ "*FUNCTION*Omo*vis_con SET ",config, Constants.TG20, print_writers, foreignLanguageOmo);
			...
			CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$LT_Manhattan$SublineGrp$Overs_Axis$Overs_Data$Title$English$txt_Tittle"
					+ "*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
			break;
		default:
			CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_Manhattan$TopLine$LogoAll$img_Logos*TEXTURE*IMAGE SET " 
					+ (config.getBroadcaster().equalsIgnoreCase(Constants.TRI_SERIES) ? Constants.TRI_SERIES_LOGO : Constants.MT20_LOGO) 
					+ inning.getBatting_team().getTeamBadge() + "\0", print_writers);
			CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_Manhattan$BarAll$Runs_Axis$Title$txt_Tittle"
					+ "*GEOM*TEXT SET RUNS\0", print_writers);
			CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_Manhattan$SublineGrp$Overs_Axis$Overs_Data$Title"
					+ "$txt_Tittle*GEOM*TEXT SET OVERS\0", print_writers);
			break;
		}
				...
				if((j*6) <= ballCount.get(1)) {
					...
					case Constants.TG20: case Constants.APLT20:
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_Manhattan$BarAll$Bar$" + j + "$obj_ScaleY" + powerPlay + "$Bar"
								+ "*TEXTURE*IMAGE SET " + Constants.TG20_BASE2 +  inning.getBatting_team().getTeamBadge() + "\0", print_writers);
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_Manhattan$BarAll$Bar$" + j + "$Wkt" + powerPlay + "$img_Base1"
								+ "*TEXTURE*IMAGE SET " + Constants.TG20_BASE1 +  inning.getBatting_team().getTeamBadge() + "\0", print_writers);
						break;
					default:
	...
	public String populateL3rdLineUpNew(String whatToProcess,int WhichSide,MatchAllData matchAllData) throws InterruptedException, StreamReadException, DatabindException, IOException
	{
		if (matchAllData == null || matchAllData.getMatch() == null || matchAllData.getMatch().getInning() == null) {
			return status;
		} 
		...
		match = matchAllData;
		lowerThird = new LowerThird("", "", "","", "","", 2, "",inning.getBatting_team().getTeamBadge(),
				null,null,null,null,null);
	...
	public String populateLTMatchPromo(String whatToProcess, int WhichSide, MatchAllData matchAllData) throws InterruptedException, JsonMappingException, JsonProcessingException {
		...
		default :
			...
			if(fixture.getDate().equalsIgnoreCase(new SimpleDateFormat("dd-MM-yyyy").format(cal_bengal.getTime()))) {
				...
				cal_bengal.add(Calendar.DATE, -1);
				if(fixture.getDate().equalsIgnoreCase(new SimpleDateFormat("dd-MM-yyyy").format(cal_bengal.getTime()))) {
					text = "UP NEXT - " + match_name + " - FROM " + fixture.getVenue();
					matchday = "UP NEXT " + " - FROM " + fixture.getVenue();
				}else {
	...
	public String populateDlsTarget(String whatToProcess,int WhichSide,MatchAllData matchAllData) throws InterruptedException, JsonMappingException, JsonProcessingException
	{
		...
		System.out.println(CricketFunctions.populateDls(matchAllData, CricketUtil.SHORT, Integer.valueOf(this_data_str.get(0))));
	...
	public String populateGeneric(String whatToProcess,int WhichSide,MatchAllData matchAllData) throws InterruptedException, JsonMappingException, JsonProcessingException
	{
		...
		switch (config.getBroadcaster().toUpperCase()) {
		case Constants.ACC:
			lowerThird = new LowerThird(whatToProcess.split(",")[2], inning.getBatting_team().getTeamName1(), data,"", 
					"", "",2,"",inning.getBatting_team().getTeamBadge(),null,null,null,null,null);
			break;
		default:
			lowerThird = new LowerThird(whatToProcess.split(",")[2], inning.getBatting_team().getTeamName2(), inning.getBatting_team().getTeamName3(),"", 
					String.valueOf(inning.getTotalRuns()) + "-" + String.valueOf(inning.getTotalWickets()), 
					CricketFunctions.OverBalls(inning.getTotalOvers(), inning.getTotalBalls()),2,"",inning.getBatting_team().getTeamBadge(),
	...
	public String PopulateL3rdHeader(String whatToProcess, int WhichSide) throws InterruptedException {
		...
		if (config.getBroadcaster().equalsIgnoreCase(Constants.TRI_SERIES)) {
		    ...
		    text2Path = Constants.TRI_SERIES_TEXT2;
				...
				config.getBroadcaster().equalsIgnoreCase(Constants.APLT20)) {
	    	...
	    	case Constants.BAN_AFG_SERIES: case Constants.ACC:
	    		...
	    		case "Control_Shift_Q":
					...
					if(lowerThird.getHeaderText().equalsIgnoreCase("CURR_PART")) {
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT$LT_ALL$All$DataAll$Side" + WhichSide 
						...
						if(inning.getPartnerships().size() == inning.getPartnerships().get(inning.getPartnerships().size() - 1).getPartnershipNumber()) {
							CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT$LT_ALL$All$DataAll$Side" + WhichSide + "$Partnership$BottomGrp$ScoreGrp$txt_Data1"
						...
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT$LT_ALL$All$DataAll$Side" + WhichSide + "$Partnership$BottomGrp$ScoreGrp$txt_Data2"
	    		...
	    		case "d": case "e":
	    	    	...
	    	    	case Constants.BAN_AFG_SERIES:
	    	    		...
	    	    		break;
	    	    	default:
	    	    		CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT$ALL_LT_LOGOGRP$Side" + WhichSide + "$Select$BothTeam$LTHomeLogoGRP$LogoIn$LLC_LogoGrp$"
		    			...
		    			CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT$ALL_LT_LOGOGRP$Side" + WhichSide + "$Select$BothTeam$LTAwayLogoGRP$LogoIn$LLC_LogoGrp$"
	        ...
	        case Constants.TRI_SERIES:  case Constants.MT20: case Constants.TG20: case Constants.APLT20:
	            ...
	            case "Control_Shift_B":
	    			...
	    			case Constants.TRI_SERIES:  case Constants.MT20: case Constants.TG20: case Constants.APLT20:
	    				...
	    				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_NextToBat$Side" + WhichSide + "$TopLine$Header_Out$Base$img_Base1*TEXTURE*IMAGE SET " 
	    						+ (lowerThird.getWhichTeamFlag().equalsIgnoreCase("LEGENDS") ? base2Path : base1Path) + lowerThird.getWhichTeamFlag() + "\0", print_writers);
		            	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_NextToBat$Side" + WhichSide + "$TopLine$Header_Out$Base$img_Base2*TEXTURE*IMAGE SET " 
	    						+ (lowerThird.getWhichTeamFlag().equalsIgnoreCase("LEGENDS") ? base1Path : base2Path) + lowerThird.getWhichTeamFlag() + "\0", print_writers);
		            	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_NextToBat$Side" + WhichSide + "$TopLine$img_Text1*TEXTURE*IMAGE SET " 
	    						+ (lowerThird.getWhichTeamFlag().equalsIgnoreCase("LEGENDS") ? text2Path : text1Path) + lowerThird.getWhichTeamFlag() + "\0", print_writers);
	            ...
	            case "Control_Shift_L":
					...
					case Constants.TRI_SERIES:  case Constants.MT20: case Constants.TG20: case Constants.APLT20:
						...
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + WhichSide + "$Team1$LogoGrp$TeamColourAll" + 
								"$img_Base1*TEXTURE*IMAGE SET " + base1Path + lowerThird.getSubTitle() + "\0", print_writers);
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + WhichSide + "$Team1$LogoGrp$TeamColourAll" + 
								"$img_Base2*TEXTURE*IMAGE SET " + base2Path + lowerThird.getSubTitle() + "\0", print_writers);
						...
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + WhichSide + "$Team2$LogoGrp$TeamColourAll" + 
								"$img_Base1*TEXTURE*IMAGE SET " + base1Path + lowerThird.getScoreText() + "\0", print_writers);
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + WhichSide + "$Team2$LogoGrp$TeamColourAll" + 
								"$img_Base2*TEXTURE*IMAGE SET " + base2Path + lowerThird.getScoreText() + "\0", print_writers);
						
						
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + WhichSide + "$Team1$LogoGrp$Logo$Loop" + 
								"$img_Logo*TEXTURE*IMAGE SET " + logoPath + lowerThird.getSubTitle() + "\0", print_writers);
						
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + WhichSide + "$Team2$LogoGrp$Logo$Loop" + 
								"$img_Logo*TEXTURE*IMAGE SET " + logoPath + lowerThird.getScoreText() + "\0", print_writers);
						...
						case Constants.TRI_SERIES:  case Constants.MT20:
							...
							break;
						case Constants.TG20: case Constants.APLT20:
							CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + WhichSide + "$Team1$TeamNameGrp"
		    					...
		    					foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.TEAM, "", multilanguagedata, 
		    							lowerThird.getHeaderText().split(",")[0], "", null, 0, foreignLanguageDataList);
		    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + WhichSide + "$Team1$TeamNameGrp"
		    							+ "$English$txt_TeamName*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
		    					
		    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + WhichSide + "$Team2$TeamNameGrp"
									+ "$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
		    					
		    					foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.TEAM, "", multilanguagedata, 
		    							lowerThird.getFirstName().split(",")[0].trim(), "", null, 0, foreignLanguageDataList);
		    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + WhichSide + "$Team2$TeamNameGrp"
		    							+ "$English$txt_TeamName*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
							break;	
						}
	            ...
	            case "Control_Shift_O":
		            	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + WhichSide + "$TopLine$Header_Out$Base" + 
								"$img_Base1*TEXTURE*IMAGE SET " + (lowerThird.getWhichTeamFlag().equalsIgnoreCase("LEGENDS") ? base2Path : base1Path) 
								+ lowerThird.getWhichTeamFlag() + "\0", print_writers);
		            	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + WhichSide + "$TopLine$Header_Out$Base" + 
								"$img_Base2*TEXTURE*IMAGE SET " + (lowerThird.getWhichTeamFlag().equalsIgnoreCase("LEGENDS") ? base1Path : base2Path) 
								+ lowerThird.getWhichTeamFlag() + "\0", print_writers);
		            	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + WhichSide + "$TopLine" + 
								"$img_Text1*TEXTURE*IMAGE SET " + (lowerThird.getWhichTeamFlag().equalsIgnoreCase("LEGENDS") ? text2Path : text1Path) 
								+ lowerThird.getWhichTeamFlag() + "\0", print_writers);
						
		            	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + WhichSide + "$TopLine$LogoAll$Side" 
		            			+ WhichSide +"$img_Logos*TEXTURE*IMAGE SET " + logoPath + lowerThird.getWhichTeamFlag() + "\0", print_writers);
					break;
	            case "Control_a":
		            	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$TopLine$LogoAll$Side" + WhichSide + 
					...
					CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$RightTextBand$TextAll$Side"+ WhichSide + 
							"$English$select_DataType*FUNCTION*Omo*vis_con SET 1\0", print_writers);
					CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$RightTextBand$TextAll$Side"+ WhichSide + 
							"$Telugu$select_DataType*FUNCTION*Omo*vis_con SET 1\0", print_writers);
					...
					switch (config.getBroadcaster().toUpperCase()) {
	    				...
	    				case Constants.TG20: case Constants.APLT20:
	    					...
	    					foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.TEAM, "", multilanguagedata, 
	    							lowerThird.getFirstName().trim(), "", null, 0, foreignLanguageDataList);
	    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$Text$Side" + WhichSide 
	    							+ "$English$txt_FirstName*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
	    					
	    					CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$TopLine$Text$Side" + WhichSide 
	    							+ "$English$txt_LastName*GEOM*TEXT SET \0", print_writers);
	    					CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$TopLine$Text$Side" + WhichSide 
	    							+ "$Telugu$txt_LastName*GEOM*TEXT SET \0", print_writers);
	    					...
	    					if(Integer.valueOf(lowerThird.getScoreText().split("-")[1])>=10) {
	    						foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata, 
	    								lowerThird.getScoreText().split("-")[0], "", null, 0, foreignLanguageDataList);
	        					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$RightTextBand$TextAll$Side" + WhichSide 
	        							+ "$English$Score$txt_Runs*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
	    					}else {
	    						foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata, 
	    								lowerThird.getScoreText(), "", null, 0, foreignLanguageDataList);
	        					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$RightTextBand$TextAll$Side" + WhichSide 
	        							+ "$English$Score$txt_Runs*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
	    					}
	    					
	    					foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,lowerThird.getBallsFacedText(),
						...
						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$RightTextBand$TextAll$Side" + WhichSide 
								+ "$English$Score$txt_Balls*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
	    					
	    					foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata, 
	    							lowerThird.getSubTitle(), "", null, 0, foreignLanguageDataList);
	    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$TopLine$Text$Side" + WhichSide 
	            ...
	            case "Control_Shift_M":
					...
					case Constants.TRI_SERIES:  case Constants.MT20: case Constants.TG20: case Constants.APLT20:
						...
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + WhichSide + "$Team1$LogoGrp$TeamColourAll" + 
								"$img_Base1*TEXTURE*IMAGE SET " + base1Path + lowerThird.getHeaderText() + "\0", print_writers);
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + WhichSide + "$Team1$LogoGrp$TeamColourAll" + 
								"$img_Base2*TEXTURE*IMAGE SET " + base2Path + lowerThird.getHeaderText() + "\0", print_writers);
						
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + WhichSide + "$Team2$LogoGrp$TeamColourAll" + 
								"$img_Base1*TEXTURE*IMAGE SET " + base1Path + lowerThird.getFirstName() + "\0", print_writers);
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + WhichSide + "$Team2$LogoGrp$TeamColourAll" + 
								"$img_Base2*TEXTURE*IMAGE SET " + base2Path + lowerThird.getFirstName() + "\0", print_writers);
						
						//home team
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + WhichSide + "$Team1$LogoGrp$Logo$Loop" + 
								"$img_Logo*TEXTURE*IMAGE SET " + logoPath + lowerThird.getHeaderText() + "\0", print_writers);
						
						//away team
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + WhichSide + "$Team2$LogoGrp$Logo$Loop" + 
								"$img_Logo*TEXTURE*IMAGE SET " + logoPath + lowerThird.getFirstName() + "\0", print_writers);
						...
						case Constants.TG20: case Constants.APLT20:
							CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + WhichSide + "$Team1$TeamNameGrp"
		    					...
		    					foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.TEAM, "", multilanguagedata, 
		    							lowerThird.getBallsFacedText(), "", null, 0, foreignLanguageDataList);
		    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + WhichSide + "$Team2$TeamNameGrp"
		    							+ "$English$txt_TeamName*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
		    					...
		    					foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata, 
		    							lowerThird.getSurName(), "", null, 0, foreignLanguageDataList);
		    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + WhichSide + "$DataPart$BottomPart"
		    							+ "$Side" + WhichSide +"$English$txt_SubTitle*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
							break;	
						}
						break;
					}	
					...
					case Constants.TG20: case Constants.APLT20:
						...
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$Text$Side"+ WhichSide + 
		    				...
		    				case Constants.TG20: case Constants.APLT20:
		    					...
		    					foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.PLAYER, CricketUtil.FULLNAME, multilanguagedata, 
		    							lowerThird.getHeaderText(), "", null, 0, foreignLanguageDataList);
		    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$Text$Side" + WhichSide 
		    							+ "$English$txt_FirstName*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
						...
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$Text$Side"+ WhichSide + 
		    				...
		    				case Constants.TG20: case Constants.APLT20:
		    					...
		    					foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.PLAYER, CricketUtil.SURNAME, multilanguagedata, 
		    							lowerThird.getSurName().trim(), "", null, 0, foreignLanguageDataList);
		    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$Text$Side" + WhichSide 
	            ...
	            case "F6": case "Control_F6": case "Shift_F6":
					...
					case Constants.TRI_SERIES:  case Constants.MT20: case Constants.TG20: case Constants.APLT20:
						...
						case Constants.TG20: case Constants.APLT20:
							...
							foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,"BALL" + 
	    							CricketFunctions.Plural(Integer.valueOf(lowerThird.getBallsFacedText())).toUpperCase(),"", null, 2,foreignLanguageDataList);
	                ...
	                case "F8": case "Alt_F8":
	                	...
	                	case Constants.TRI_SERIES:  case Constants.MT20: 
	                		if(lowerThird.getWhichTeamFlag() != null) {
	    						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper_Center$TopLine$LogoAll$Side" + WhichSide + 
	    								"$img_Logos*TEXTURE*IMAGE SET " + (config.getBroadcaster().equalsIgnoreCase(Constants.TRI_SERIES) ? 
	        									Constants.TRI_SERIES_LOGO : Constants.MT20_LOGO) + lowerThird.getWhichTeamFlag() + "\0", print_writers);
...
//									+ "$select_DataType$Name$txt_LastName*GEOM*TEXT SET " +  lowerThird.getSurName() + "\0", print_writers);
	                	...
	                	case Constants.TG20: case Constants.APLT20:
	                		...
	                		CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper_Center$TopLine$img_Text1$Side" + WhichSide 
	    							+ "$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
	                		
	    					foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.PLAYER, CricketUtil.FULLNAME, multilanguagedata, 
	    							lowerThird.getSubTitle().trim(), "", null, 0, foreignLanguageDataList);
	    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper_Center$TopLine$img_Text1$Side" + WhichSide 
	    							+ "$English$txt_FirstName*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
...
//	    							lowerThird.getSurName().trim(), "", null, 0, foreignLanguageDataList);
//	    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper$TopLine$img_Text1$Side" + WhichSide 
//	    							+ "$English$txt_LastName*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
	                ...
	                case "F10":
	    				...
	    				case Constants.TRI_SERIES:  case Constants.MT20: case Constants.TG20: case Constants.APLT20:
	    					...
	    					case Constants.APLT20:
	    						if(!lowerThird.getWhichSponsor().isEmpty() && !lowerThird.getWhichTeamFlag().isEmpty()) {
		    						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper_Center$TopLine$LogoAll$Side" + WhichSide + 
		    								"$img_Logos*TEXTURE*IMAGE SET " + Constants.TG20_LOGO_BW + lowerThird.getWhichTeamFlag() + "\0", print_writers);
		    					}else if(!lowerThird.getWhichSponsor().isEmpty() && lowerThird.getWhichTeamFlag().isEmpty()) {
		    						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper_Center$TopLine$LogoAll$Side" + WhichSide + 
		    								"$img_Logos*TEXTURE*IMAGE SET " + Constants.TG20_LOGO_BW + lowerThird.getWhichSponsor() + "\0", print_writers);
		    					}else if(lowerThird.getWhichSponsor().isEmpty() && !lowerThird.getWhichTeamFlag().isEmpty()) {
		    						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper_Center$TopLine$LogoAll$Side" + WhichSide + 
		    								"$img_Logos*TEXTURE*IMAGE SET " + Constants.TG20_LOGO_BW + lowerThird.getWhichTeamFlag() + "\0", print_writers);
		    					}else {
		    						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper_Center$TopLine$LogoAll$Side" + WhichSide + 
		    								"$img_Logos*TEXTURE*IMAGE SET " + Constants.TG20_LOGO_BW + "EVENT" + "\0", print_writers);
		    					}
	    						break;
	    					default:
	    						if(!lowerThird.getWhichSponsor().isEmpty() && !lowerThird.getWhichTeamFlag().isEmpty()) {
		    						...
		    						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper_Center$TopLine$LogoAll$Side" + WhichSide + 
		    								"$img_Logos*TEXTURE*IMAGE SET " + logoPath + lowerThird.getWhichSponsor() + "\0", print_writers);
		    					}else if(lowerThird.getWhichSponsor().isEmpty() && !lowerThird.getWhichTeamFlag().isEmpty()) {
		    						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper_Center$TopLine$LogoAll$Side" + WhichSide + 
		    								"$img_Logos*TEXTURE*IMAGE SET " + logoPath + lowerThird.getWhichTeamFlag() + "\0", print_writers);
		    					}else {
		    						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper_Center$TopLine$LogoAll$Side" + WhichSide + 
		    								"$img_Logos*TEXTURE*IMAGE SET " + logoPath + "EVENT" + "\0", print_writers);
		    					}
	    						break;
	    					}
	    			...
	    			case "Shift_F3":
						...
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$RightTextBand$TextAll$Side"+ WhichSide + 
		    				...
		    				case Constants.TG20: case Constants.APLT20:
		    					...
		    					foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata, 
		    							lowerThird.getHeaderText().trim(), "", null, 0, foreignLanguageDataList);
		    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$Text$Side" + WhichSide 
		    							+ "$English$txt_Designation*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
		    					
		    					foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,lowerThird.getSubTitle(),
	    							    "", null, 1,foreignLanguageDataList);
    							...
    							if(Integer.valueOf(lowerThird.getBallsFacedText()) >= 10) {
    								foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata, 
    		    						...
    		    						foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata, 
    		    								lowerThird.getScoreText() + "-" + lowerThird.getBallsFacedText(), "", null, 0, foreignLanguageDataList);
    			    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$RightTextBand$TextAll$Side" + WhichSide 
    			    							+ "$English$Score$txt_Runs*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
    		    					}
		    					break;
						...
						switch (config.getBroadcaster().toUpperCase()) {
		    				...
		    				case Constants.TG20: case Constants.APLT20:
		    					...
		    					foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata, 
		    							lowerThird.getSubTitle(), "", null, 0, foreignLanguageDataList);
	    						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$TopLine$Text$Side" + WhichSide 
		    							+ "$English$select_DataType$txt_Designation*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
	    			...
	    			case "Shift_F5": case "Shift_F9":
	    				...
	    				case "Shift_F5":
	    					...
	    					case Constants.TG20: case Constants.APLT20:
    							...
    							foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,"BALL" + 
	    								CricketFunctions.Plural(Integer.valueOf(lowerThird.getBallsFacedText())).toUpperCase(),
    									"", null, 2,foreignLanguageDataList);
	    			...
	    			case "d":  case "e":
						...
						switch (config.getBroadcaster().toUpperCase()) {
		    				...
		    				case Constants.TG20: case Constants.APLT20:
		    					...
		    					foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.TEAM, "", multilanguagedata, 
		    							teamNameAsCity, "", null, 0, foreignLanguageDataList);
		    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$Text$Side" + WhichSide 
		    							+ "$English$txt_FirstName*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
	    			...
	    			case "Alt_Shift_F12":
						...
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$Text$Side"+ WhichSide + 
		    				...
		    				case Constants.TG20: case Constants.APLT20:
		    					...
		    					foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.TEAM, "", multilanguagedata, 
		    							lowerThird.getFirstName(), "", null, 0, foreignLanguageDataList);
		    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$Text$Side" + WhichSide 
		    							+ "$English$txt_FirstName*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
	    			...
	    			case "Control_F3"://Comparison
	                    ...
	                    case Constants.TRI_SERIES:  case Constants.MT20: case Constants.TG20: case Constants.APLT20:
							...
							CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_Comparison$Side0" + WhichSide + "$DataPart$BottomPart$HomeTeam" + 
									"$img_Base2*TEXTURE*IMAGE SET " + base2Path + lowerThird.getFirstName() + "\0", print_writers);
							CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_Comparison$Side0" + WhichSide + "$DataPart$BottomPart$HomeTeam" + 
									"$img_Text2*TEXTURE*IMAGE SET " + text2Path + lowerThird.getFirstName() + "\0", print_writers);
							...
							CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_Comparison$Side0" + WhichSide + "$Team2$LogoGrp$TeamColourAll" + 
									"$img_Base1*TEXTURE*IMAGE SET " + base1Path + lowerThird.getSurName() + "\0", print_writers);
							CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_Comparison$Side0" + WhichSide + "$Team2$LogoGrp$TeamColourAll" + 
									"$img_Base2*TEXTURE*IMAGE SET " + base2Path + lowerThird.getSurName() + "\0", print_writers);
							...
							CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_Comparison$Side0" + WhichSide + "$DataPart$BottomPart$AwayTeam" + 
									"$img_Base2*TEXTURE*IMAGE SET " + base2Path + lowerThird.getSurName() + "\0", print_writers);
							CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_Comparison$Side0" + WhichSide + "$DataPart$BottomPart$AwayTeam" + 
									"$img_Text2*TEXTURE*IMAGE SET " + text2Path + lowerThird.getSurName() + "\0", print_writers);
	                 	   ...
	                 	   CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_Comparison$Side0" + WhichSide + "$Team1$LogoGrp$Logo$Loop$img_Logo*TEXTURE*IMAGE SET " + 
	                 			  logoPath + lowerThird.getFirstName() + "\0", print_writers);
	                 	  //bating team flag
	                 	   CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_Comparison$Side0" + WhichSide + "$Team2$LogoGrp$Logo$Loop$img_Logo*TEXTURE*IMAGE SET " + 
	                 			  logoPath + lowerThird.getSurName() + "\0", print_writers);
	                 	  ...
	                 	  switch (config.getBroadcaster().toUpperCase()) {
		                 	   ...
		                 	   CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_Comparison$Side0" + WhichSide + "$Team2$TeamNameGrp$TeamName$txt_TeamName*GEOM*TEXT SET " + 
			    					...
			    					break;
			    				case Constants.TG20: case Constants.APLT20:
			    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$Lt_Comparison$Side0" + WhichSide + "$Team1$TeamNameGrp$TeamName"
			    							+ "$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
			    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$Lt_Comparison$Side0" + WhichSide + "$Team2$TeamNameGrp$TeamName"
			    							+ "$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
			                		
			    					
			    					foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.TEAM, "", multilanguagedata, 
			    							lowerThird.getLeftText()[0], "", null, 0, foreignLanguageDataList);
			    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$Lt_Comparison$Side0" + WhichSide + "$Team1$TeamNameGrp$TeamName"
			    							+ "$English$txt_TeamName*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
			    					
			    					foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.TEAM, "", multilanguagedata, 
			    							lowerThird.getLeftText()[3], "", null, 0, foreignLanguageDataList);
			    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$Lt_Comparison$Side0" + WhichSide + "$Team2$TeamNameGrp$TeamName"
			    							+ "$English$txt_TeamName*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
			    					break;
		    					}
	...
	private void PopulateL3rdBaseColor(String whatToProcess, int whichSide) {
        ...
        case Constants.TRI_SERIES:  case Constants.MT20: case Constants.TG20: case Constants.APLT20:
            switch (whatToProcess) {
            	...
            	case "Alt_F1": case "Alt_F2":
            		CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$Header_Out$Side" + whichSide + 
                			"$img_Base1*TEXTURE*IMAGE SET " + base1Path + l3griff.getWhichTeamFlag() +"\0", print_writers);
                	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$Text$Side" + whichSide + "$img_Text1" + 
							"*TEXTURE*IMAGE SET " + text1Path + l3griff.getWhichTeamFlag() +"\0", print_writers);
                	...
                	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$RightTextBand$Base$Side" + whichSide + 
							"$img_Base2*TEXTURE*IMAGE SET " + base2Path + l3griff.getWhichTeamFlag() +"\0", print_writers);
                	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$RightTextBand$TextAll$Side" + whichSide + 
							"$img_Text2*TEXTURE*IMAGE SET " + text2Path + l3griff.getWhichTeamFlag() +"\0", print_writers);
					
                	break;
                	
                case "F8": case "Alt_F8":
                	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper$TopLine$TeamColour$Side" + whichSide + 
							"$img_Base1*TEXTURE*IMAGE SET " + base1Path + lowerThird.getWhichTeamFlag() +"\0", print_writers);
                	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper$TopLine$TeamColour$Side" + whichSide + 
							"$img_Base2*TEXTURE*IMAGE SET " + base2Path + lowerThird.getWhichTeamFlag() +"\0", print_writers);
                	break;
                case "F10":
                	if(!lowerThird.getWhichSponsor().isEmpty() && !lowerThird.getWhichTeamFlag().isEmpty()) {
                		CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper$TopLine$TeamColour$Side" + whichSide + 
						...
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper$TopLine$TeamColour$Side" + whichSide + 
								"$img_Base1*TEXTURE*IMAGE SET " + base1Path + lowerThird.getWhichSponsor() +"\0", print_writers);
	                	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper$TopLine$TeamColour$Side" + whichSide + 
								"$img_Base2*TEXTURE*IMAGE SET " + base2Path + lowerThird.getWhichSponsor() +"\0", print_writers);
					}else if(lowerThird.getWhichSponsor().isEmpty() && !lowerThird.getWhichTeamFlag().isEmpty()) {
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper$TopLine$TeamColour$Side" + whichSide + 
								"$img_Base1*TEXTURE*IMAGE SET " + base1Path + lowerThird.getWhichTeamFlag() +"\0", print_writers);
	                	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper$TopLine$TeamColour$Side" + whichSide + 
						...
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper$TopLine$TeamColour$Side" + whichSide + 
								"$img_Base1*TEXTURE*IMAGE SET " + base1Path + "EVENT" +"\0", print_writers);
	                	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper$TopLine$TeamColour$Side" + whichSide + 
								"$img_Base2*TEXTURE*IMAGE SET " + base2Path + "EVENT" +"\0", print_writers);
					}
                	break;	
                ...
                default:
                	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$Header_Out$Side" + whichSide + 
                			"$img_Base1*TEXTURE*IMAGE SET " + base1Path + lowerThird.getWhichTeamFlag() +"\0", print_writers);
                	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$Text$Side" + whichSide + "$img_Text1" + 
							"*TEXTURE*IMAGE SET " + text1Path + lowerThird.getWhichTeamFlag() +"\0", print_writers);
                	
                	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$RightTextBand$Base$Side" + whichSide + 
							"$img_Base2*TEXTURE*IMAGE SET " + base2Path + lowerThird.getWhichTeamFlag() +"\0", print_writers);
                	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$TopLine$RightTextBand$TextAll$Side" + whichSide + 
							"$img_Text2*TEXTURE*IMAGE SET " + text2Path + lowerThird.getWhichTeamFlag() +"\0", print_writers);
	...
	public String L3rdBodyPart(int whichSide, String whatToProcess) throws JsonMappingException, JsonProcessingException, InterruptedException {
		...
		case "Control_Shift_Q":
			...
			case Constants.ACC:
				...
				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT$LT_ALL$All$DataAll$Side" + whichSide + "$Partnership$BottomGrp$RestDataGrp$Player1$"
				...
				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT$LT_ALL$All$DataAll$Side" + whichSide + "$Partnership$BottomGrp$RestDataGrp$Player2$"
				...
				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT$LT_ALL$All$DataAll$Side" + whichSide + "$Partnership$BottomGrp$RestDataGrp$Contribution$"
		...
		switch (config.getBroadcaster().toUpperCase()) {
	        ...
	        case Constants.TRI_SERIES:  case Constants.MT20: case Constants.TG20: case Constants.APLT20:
	            ...
	            case "Control_F2":
	            	...
	            	if(omoSize >=5) {
	            		CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_BowlingOption$Side" + whichSide + "$Select"
	            	...
	            	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_BowlingOption$Side1" + selectOption
	    					+ "$TopLine$HeadText$txt_Header*GEOM*TEXT SET " + team.getTeamName1() + " -" + " BOWLING OPTIONS" + "\0", print_writers);
	            	
	            	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_BowlingOption$Side1" + selectOption
	    					+ "$TopLine$LogoAll$img_Logos*TEXTURE*IMAGE SET " + logoPath + team.getTeamBadge() + "\0", print_writers);
	            	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_BowlingOption$Side1" + selectOption + "$TopLine$HeadText$img_Text1"
	            			+ "*TEXTURE*IMAGE SET " + (team.getTeamBadge().equalsIgnoreCase("LEGENDS")?text2Path:text1Path) + team.getTeamBadge() + "\0", print_writers);
	            	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_BowlingOption$Side1" + selectOption + "$TopLine$Header_Out$img_Base1"
	            			+ "*TEXTURE*IMAGE SET " + (team.getTeamBadge().equalsIgnoreCase("LEGENDS")?base2Path:base1Path) + team.getTeamBadge() + "\0", print_writers);
	            	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_BowlingOption$Side1" + selectOption + "$TopLine$Header_Out$img_Base2"
	            			+ "*TEXTURE*IMAGE SET " + (team.getTeamBadge().equalsIgnoreCase("LEGENDS")?base1Path:base2Path) + team.getTeamBadge() + "\0", print_writers);
	            			...
	            			case Constants.TG20: case Constants.APLT20:
	            				if(bc.getRole().equalsIgnoreCase(CricketUtil.BOWLER)) {
	  	    						if(bc.getBowlingStyle() == null) {
	  	    							CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_BowlingOption$Side1"+ selectOption + "$TopLine$img_Text1$" + count + 
	  	    									"$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "Bowler" + "\0", print_writers);
	  	    						}else {
	  	    							switch (bc.getBowlingStyle().toUpperCase()) {
	  	    							case "RFM": case "RF": case "RMF": case "RM": case "RSM": case "LF": case "LFM": case "LMF": case "LM":
	  	    								CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_BowlingOption$Side1"+ selectOption + "$TopLine$img_Text1$" + count + 
		  	    									"$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "FastBowler" + "\0", print_writers);
	  	    								break;

	  	    							case "LSL": case "WSL": case "LCH": case "RLG": case "WSR": case "LSO":
	  	    								CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_BowlingOption$Side1"+ selectOption + "$TopLine$img_Text1$" + count + 
		  	    									"$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "SpinBowler" + "\0", print_writers);
	  	    								break;
	  	    							case "ROB":
	  	    								CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_BowlingOption$Side1"+ selectOption + "$TopLine$img_Text1$" + count + 
		  	    									"$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "Off_Spin" + "\0", print_writers);
	  	    								break;
	  	    							case "RLB":
	  	    								CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_BowlingOption$Side1"+ selectOption + "$TopLine$img_Text1$" + count + 
		  	    									"$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "Leg_Spin" + "\0", print_writers);
	  	    								break;	
	  	    							}
	  	    						}
	  	    						...
	  	    						if(bc.getBowlingStyle() == null) {
	  	    							if(bc.getBattingStyle().equalsIgnoreCase("LHB")) {
	  	    								CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_BowlingOption$Side1"+ selectOption + "$TopLine$img_Text1$" + count + 
		  	    									"$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "Off_Spin_Allrounder_Left" + "\0", print_writers);
	  	    							}else {
	  	    								CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_BowlingOption$Side1"+ selectOption + "$TopLine$img_Text1$" + count + 
		  	    									"$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "FastBowlerAllrounder" + "\0", print_writers);
	  	    							}
	  	    							...
	  	    							case "RFM": case "RF": case "RMF": case "RM": case "RSM": case "LF": case "LFM": case "LMF": case "LM":
	  	    								if(bc.getBattingStyle().equalsIgnoreCase("LHB")) {
	  	    									CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_BowlingOption$Side1"+ selectOption + "$TopLine$img_Text1$" + count + 
	  		  	    									"$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "Pace_BowlerAllrounerLeftHand" + "\0", print_writers);
	  	    								}else {
	  	    									CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_BowlingOption$Side1"+ selectOption + "$TopLine$img_Text1$" + count + 
	  		  	    									"$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "FastBowlerAllrounder" + "\0", print_writers);
	  	    								}
	  	    							...
	  	    							case "ROB": case "RLB": case "LSL": case "WSL": case "LCH": case "RLG": case "WSR": case "LSO":
	  	    								if(bc.getBattingStyle().equalsIgnoreCase("LHB")) {
	  	    									...
	  	    									CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_BowlingOption$Side1"+ selectOption + "$TopLine$img_Text1$" + count + 
	  		  	    									"$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "Off_Spin_Bat" + "\0", print_writers);
	  	    								}
	  	    								break;
	  	    							}
	  	    						}
	            	...
	            	break;
	            case "Shift_I":
	            	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$LogoAll$Side1$img_Logos"
	    					+ "*TEXTURE*IMAGE SET " + Constants.TG20_LOGO + lowerThird.getWhichTeamFlag() + " \0", print_writers);
	            	
//	    			CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$LogoAll$Side1$img_Logos"
//	    					+ "*TEXTURE*IMAGE SET " + Constants.MT20_LOGO + lowerThird.getWhichTeamFlag() + " \0", print_writers);
//	    			
//	    			CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$TeamColour$Side1$img_Base1"
//	    					+ "*TEXTURE*IMAGE SET " + Constants.MT20_BASE1 + lowerThird.getWhichTeamFlag() + " \0", print_writers);
//	    			CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$TeamColour$Side1$img_Base2"
//	    					+ "*TEXTURE*IMAGE SET " + Constants.MT20_BASE2 + lowerThird.getWhichTeamFlag() + " \0", print_writers);
	    			
//	    			if(impact_photo.equalsIgnoreCase("NO")) {
//	    				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$img_Text1$Imapct$Side1$Image$img_Player*ACTIVE SET 0\0", print_writers);
//	    				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$img_Text1$Sub$Side1$Image$img_Player*ACTIVE SET 0\0", print_writers);
//	    			}else {
//	    				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$img_Text1$Imapct$Side1$Image$img_Player*ACTIVE SET 1\0", print_writers);
//	    				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$img_Text1$Sub$Side1$Image$img_Player*ACTIVE SET 1\0", print_writers);
//	    			}
	    			//in
	    			CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$img_Text1$Imapct$Name$txt_FirstName*GEOM*TEXT SET " 
	    					+ lowerThird.getSubTitle() + "\0", print_writers);
	    			CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$img_Text1$Imapct$Name$txt_LastName*GEOM*TEXT SET " 
	    					+ (lowerThird.getScoreText() != null?lowerThird.getScoreText():"") + "\0", print_writers);
	    			CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$img_Text1$Imapct$Side" + whichSide + "$Select_Subline"
	    					+ "*FUNCTION*Omo*vis_con SET 0\0", print_writers);
	    			CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$img_Text1$Imapct$txt_Designation"
	    					+ "*GEOM*TEXT SET " + "IMPACT IN" + "\0", print_writers);
	    			
	    			
//	    			if(config.getPrimaryIpAddress().equalsIgnoreCase(Constants.LOCALHOST)) {
//	    				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$img_Text1$Imapct$Image$img_Player*TEXTURE*IMAGE SET " 
//	    						+ Constants.MT20_LOCAL_PHOTO_PATH + "\\\\" + Constants.CENTER  + "\\\\" + "BLANK" + CricketUtil.PNG_EXTENSION 
//	    						+ " \0", print_writers);
//	    				
//	    				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$img_Text1$Imapct$Image$img_Player*TEXTURE*IMAGE SET " 
//	    						+ Constants.MT20_LOCAL_PHOTO_PATH + "\\\\" + team.getTeamBadge()+ "\\\\" + Constants.CENTER + "\\" + lowerThird.getBallsFacedText() + CricketUtil.PNG_EXTENSION 
//	    						+ " \0", print_writers);
//	    				
//	    			}else {
//	    				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$img_Text1$Imapct$Image$img_Player*TEXTURE*IMAGE SET " 
//	    						+ "\\\\" + config.getPrimaryIpAddress() + Constants.MT20_PHOTO_PATH + "\\\\" + team.getTeamBadge() + "\\\\" + Constants.CENTER + "\\" + "BLANK" + CricketUtil.PNG_EXTENSION 
//	    						+ " \0", print_writers);
//	    				
//	    				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$img_Text1$Imapct$Image$img_Player*TEXTURE*IMAGE SET " 
//	    						+ "\\\\" + config.getPrimaryIpAddress() + Constants.MT20_PHOTO_PATH + "\\\\" + team.getTeamBadge() + "\\\\" + Constants.CENTER + "\\" + lowerThird.getBallsFacedText() + CricketUtil.PNG_EXTENSION 
//	    						+ " \0", print_writers);
//	    			}
	    			
	    			//out
	    			
	    			CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$img_Text1$Sub$Name$txt_FirstName*GEOM*TEXT SET " 
	    					+ lowerThird.getHeaderText() + "\0", print_writers);
	    			CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$img_Text1$Sub$Name$txt_LastName*GEOM*TEXT SET " 
	    					+ (lowerThird.getFirstName() != null?lowerThird.getFirstName():"") + "\0", print_writers);
	    			CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$img_Text1$Imapct$Side" + whichSide + "$Select_Subline"
	    					+ "*FUNCTION*Omo*vis_con SET 0\0", print_writers);
	    			CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$img_Text1$Sub$txt_Designation"
	    					+ "*GEOM*TEXT SET " + "SUB OUT" + "\0", print_writers);
	    			
//	    			if(config.getPrimaryIpAddress().equalsIgnoreCase(Constants.LOCALHOST)) {
//	    				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$img_Text1$Sub$Image$img_Player*TEXTURE*IMAGE SET " 
//	    						+ Constants.MT20_LOCAL_PHOTO_PATH + "\\\\" + "BLANK" + CricketUtil.PNG_EXTENSION 
//	    						+ " \0", print_writers);
//	    				
//	    				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$img_Text1$Sub$Image$img_Player*TEXTURE*IMAGE SET " 
//	    						+ Constants.MT20_LOCAL_PHOTO_PATH + "\\\\" + team.getTeamBadge()+ "\\\\" + Constants.CENTER + "\\" + lowerThird.getSurName() + CricketUtil.PNG_EXTENSION 
//	    						+ " \0", print_writers);
//	    			}else {
//	    				
//	    				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$img_Text1$Sub$Image$img_Player*TEXTURE*IMAGE SET " 
...
//	    						+ " \0", print_writers);
//	    				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_ImpactSub$img_Text1$Sub$Image$img_Player*TEXTURE*IMAGE SET " 
//	    						+ "\\\\" + config.getPrimaryIpAddress() + Constants.MT20_PHOTO_PATH + "\\\\" + team.getTeamBadge()+ "\\\\" + Constants.CENTER + "\\" + lowerThird.getSurName() + CricketUtil.PNG_EXTENSION 
//	    						+ " \0", print_writers);
//	    			}
	    			break;
		...
		//    							+ "$select_Subline$1$Title$txt_" + (i+1) + "*GEOM*TEXT SET " + "v " + l3griff.getTopTitlesText()[i] + "\0", print_writers);
	            ...
	            case "Alt_d": case "Alt_f":
					...
					CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide +
							"$select_Subline$4$Title*ACTIVE SET 0 \0", print_writers);
					CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide +
							"$1$Right*ACTIVE SET 1\0", print_writers);
					
					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide 
							+ "$1$Left$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide 
							+ "$1$Right$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
					
					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$SublineGrp$Side" + whichSide 
							+ "$select_Subline$1$Left$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$SublineGrp$Side" + whichSide 
...
//								    "", null, 3,foreignLanguageDataList);
//							foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,"RUNS",
//								    "", null, 4,foreignLanguageDataList);
//							foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,lowerThird.getLeftText()[1].toUpperCase().split("RUNS")[1].trim(),
//								    "", null, 5,foreignLanguageDataList);
//						}else {
//							foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,lowerThird.getLeftText()[1].toUpperCase().split("RUN")[0].split("ARE")[1].trim(),
//								    "", null, 3,foreignLanguageDataList);
//							foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,"RUN",
//								    "", null, 4,foreignLanguageDataList);
//							foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,lowerThird.getLeftText()[1].toUpperCase().split("RUN")[1].trim(),
//								    "", null, 5,foreignLanguageDataList);
//						}
						foreignLanguageData.add(CricketFunctions.MergeForeignLanguageDataListToSingleObject(foreignLanguageDataList));
					}
					...
					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide 
							+ "$select_Subline$1$Right$English$txt_1*GEOM*TEXT SET ",config, Constants.BCCI, print_writers, foreignLanguageData);
					
					CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide +
							"$select_Subline$1$Right$English$txt_1*GEOM*TEXT SET " + lowerThird.getLeftText()[1].toUpperCase() + "\0", print_writers);
					...
					case Constants.TRI_SERIES:  case Constants.MT20: case Constants.TG20: case Constants.APLT20:
						...
						switch (config.getBroadcaster().toUpperCase()) {
		    				...
		    				case Constants.TG20: case Constants.APLT20:
		    					...
		    					foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata, lowerThird.getLeftText()[0].trim().toUpperCase(), 
	    								"", null, 0, foreignLanguageDataList);
	    						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide 
	    								+ "$select_Subline$1$Left$English$txt_1*GEOM*TEXT SET ",config, Constants.BCCI, print_writers, foreignLanguageData);
								...
								if(!lowerThird.getRightText()[0].equalsIgnoreCase("WITHOUTEND")) {
									...
									switch (config.getBroadcaster().toUpperCase()) {
					    				...
					    				case Constants.TG20: case Constants.APLT20:
					    					...
					    					foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata, lowerThird.getRightText()[0].trim().toUpperCase(), 
				    								"", null, 0, foreignLanguageDataList);
				    						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide 
				    								+ "$select_Subline$1$Right$English$txt_1*GEOM*TEXT SET ",config, Constants.BCCI, print_writers, foreignLanguageData);
									...
									CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Side" + whichSide 
											+ "$select_Subline$1$Right$English$txt_1*GEOM*TEXT SET " +  "" + "\0", print_writers);
									CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Side" + whichSide 
											+ "$select_Subline$1$Right$Telugu$txt_1*GEOM*TEXT SET " +  "" + "\0", print_writers);
								}
								break;
					...
					case Constants.TRI_SERIES:  case Constants.MT20: case Constants.TG20: case Constants.APLT20:
						...
						switch (config.getBroadcaster().toUpperCase()) {
		    				case Constants.TRI_SERIES:  case Constants.MT20:
		    					...
		    					break;
		    				case Constants.TG20: case Constants.APLT20:
		    					foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,"FOURS",
	    							    "", null, 1,foreignLanguageDataList);
		    					foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,": ",
	    							    "", null, 2,foreignLanguageDataList);
		    					foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,lowerThird.getLeftText()[0],
		    					...
		    					foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,"  |  ",
	    							    "", null, 4,foreignLanguageDataList);
		    					foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,"SIXES",
	    							    "", null, 5,foreignLanguageDataList);
		    					foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,": ",
	    							    "", null, 6,foreignLanguageDataList);
		    					foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,lowerThird.getLeftText()[1],
	    							    "", null, 7,foreignLanguageDataList);
							...
							foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,"STRIKE RATE",
    							    "", null, 1,foreignLanguageDataList);
		    					foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,": "+lowerThird.getRightText()[0],
	    							    "", null, 2,foreignLanguageDataList);
						...
						switch (config.getBroadcaster().toUpperCase()) {
		    				...
		    				case Constants.TG20: case Constants.APLT20:
		    					...
		    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide 
		    							+ "$1$Left$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
    							CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide 
		    							+ "$2$Left$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
								...
								if(battingCard.getWasHowOutFielderSubstitute().equalsIgnoreCase("NO")) {
									dichowOut = CricketFunctions.generateBattingCardForeignLanguage(battingCard.getHowOut(), battingCard.getHowOutPartOne(), battingCard.getHowOutPartTwo(), 
											"", multilanguagedata);
								}else {
									dichowOut = CricketFunctions.generateBattingCardForeignLanguage(battingCard.getHowOut(), battingCard.getHowOutPartOne(), battingCard.getHowOutPartTwo(), 
		    					...
		    					foreignLanguageData.add(new ForeignLanguageData(dichowOut.getEnglishText().replace("|", ""), dichowOut.getHindiText().replace("|", ""), dichowOut.getTamilText().replace("|", ""), 
		    							dichowOut.getTeluguText().replace("|", "")));
							CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide
										+ "$select_Subline$1$Left$English$txt_1*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
		    					...
		    					foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,": ",
	    							    "", null, 6,foreignLanguageDataList);
		    					foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,lowerThird.getLeftText()[2],
	    							    "", null, 7,foreignLanguageDataList);
		    					foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,"  |   ",
	    							    "", null, 8,foreignLanguageDataList);
		    					foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,"DOTS",
	    							    "", null, 9,foreignLanguageDataList);
		    					foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,": ",
	    							    "", null, 10,foreignLanguageDataList);
		    					foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,lowerThird.getLeftText()[3],
	    							    "", null, 11,foreignLanguageDataList);
		    					foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,"  |   ",
	    							    "", null, 12,foreignLanguageDataList);
		    					foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,"STRIKE RATE",
	    							    "", null, 13,foreignLanguageDataList);
		    					foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,": ",
	    							    "", null, 14,foreignLanguageDataList);
		    					foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,lowerThird.getLeftText()[4],
	    							    "", null, 15,foreignLanguageDataList);
	
							foreignLanguageData.add(CricketFunctions.MergeForeignLanguageDataListToSingleObject(foreignLanguageDataList));
							
							CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide 
									+ "$select_Subline$2$Left$English$txt_1*GEOM*TEXT SET ",config, Constants.BCCI, print_writers, foreignLanguageData);
    					}
					break;
					}
					break;
					...
					case Constants.TRI_SERIES:  case Constants.MT20: case Constants.TG20: case Constants.APLT20:
						...
						switch (config.getBroadcaster().toUpperCase()) {
		    				...
		    				case Constants.TG20: case Constants.APLT20:
		    					...
		    					if(battingCardList.get(battingCardList.size()-1).getWasHowOutFielderSubstitute() != null) {
								if(battingCardList.get(battingCardList.size()-1).getWasHowOutFielderSubstitute().equalsIgnoreCase("NO")) {
									dichowOut = CricketFunctions.generateBattingCardForeignLanguage(battingCardList.get(battingCardList.size()-1).getHowOut(), battingCardList.get(battingCardList.size()-1).getHowOutPartOne(), battingCardList.get(battingCardList.size()-1).getHowOutPartTwo(), 
											"", multilanguagedata);
								}else {
									dichowOut = CricketFunctions.generateBattingCardForeignLanguage(battingCardList.get(battingCardList.size()-1).getHowOut(), battingCardList.get(battingCardList.size()-1).getHowOutPartOne(), battingCardList.get(battingCardList.size()-1).getHowOutPartTwo(), 
					...
					case "F10":
						switch (config.getBroadcaster().toUpperCase()) {
		    				...
		    				case Constants.TG20: case Constants.APLT20:
	    						...
	    						foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata, lowerThird.getLeftText()[0], 
	    								"", null, 0, foreignLanguageDataList);
	    						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper_Center$TopLine$img_Text1$Side" + whichSide 
	    								+ "$English$txt_Designation*GEOM*TEXT SET ",config, Constants.BCCI, print_writers, foreignLanguageData);
	                ...
	                case "Control_a":
    					...
    					switch (config.getBroadcaster().toUpperCase()) {
	    					...
	    					case Constants.TG20: case Constants.APLT20:
	    						...
	    						for(int iStat = 0; iStat < lowerThird.getTitlesText().length; iStat++) {
	    							CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$SublineGrp$Side" + whichSide 
			    							+ "$select_Subline$1$Title$" + (iStat + 1) + "$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
	    							CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide 
			    							+ "$1$Left$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
	    							CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide 
			    							+ "$2$Left$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
	    							
	    							if(lowerThird.getTitlesText()[iStat].contains("/OVER")) {
//	    								foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata, "/" + lowerThird.getTitlesText()[iStat].split("/")[1], 
//			    								"", null, 0, foreignLanguageDataList);
	    								
	    								foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,lowerThird.getTitlesText()[iStat].split("/")[0],
			    							    "", null, 1,foreignLanguageDataList);
			    						
		    							foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,"/" + lowerThird.getTitlesText()[iStat].split("/")[1],
		    									"", null, 2,foreignLanguageDataList);

		    							foreignLanguageData.add(CricketFunctions.MergeForeignLanguageDataListToSingleObject(foreignLanguageDataList));

	    							}else if(lowerThird.getTitlesText()[iStat].contains("CURRENT")) {
	    								foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,lowerThird.getTitlesText()[iStat].split(" ")[0],
			    							    "", null, 1,foreignLanguageDataList);
		    						...
		    						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$SublineGrp$Side" + whichSide 
		    								+ "$select_Subline$1$Title$" + (iStat + 1) + "$English$txt_1*GEOM*TEXT SET ",config, Constants.BCCI, print_writers, foreignLanguageData);
	        						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$SublineGrp$Side" + whichSide 
	        							+ "$select_Subline$2$Stat$txt_" + (iStat + 1) + "*GEOM*TEXT SET " + lowerThird.getStatsText()[iStat] + "\0", print_writers);
	        					}
	    						...
	    						foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata, lowerThird.getLeftText()[0], 
	    								"", null, 0, foreignLanguageDataList);
	    						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide 
	    								+ "$1$Left$English$txt_1*GEOM*TEXT SET ",config, Constants.BCCI, print_writers, foreignLanguageData);
	    						
	    						foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata, lowerThird.getLeftText()[1], 
	    								"", null, 0, foreignLanguageDataList);
	    						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide 
	    								+ "$2$Left$English$txt_1*GEOM*TEXT SET ",config, Constants.BCCI, print_writers, foreignLanguageData);
	    					break;	
	    				}
	                    break; 
	                case "F8": case "Alt_F8":
	    				switch (config.getBroadcaster().toUpperCase()) {
	    					case Constants.TRI_SERIES:  case Constants.MT20:
		    					...
		    					if(lowerThird.getLeftText()[0].equalsIgnoreCase("Captain") || lowerThird.getLeftText()[0].equalsIgnoreCase("Captain Wicket-Keeper") ||
		    							lowerThird.getLeftText()[0].equalsIgnoreCase("Wicket-Keeper")) {
		    						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper_Center$TopLine$img_Text1$Side" + whichSide 
			    							+ "$txt_Designation*GEOM*TEXT SET " + lowerThird.getLeftText()[0].toUpperCase() + " " + lowerThird.getSubTitle() + "\0", print_writers);
//		    						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide +
...
//		    								"$select_Subline$1$Left$txt_1*GEOM*TEXT SET " + lowerThird.getLeftText()[0].toUpperCase() + "\0", print_writers);
	    					...
	    					case Constants.TG20: case Constants.APLT20:
		    					...
		    					if(lowerThird.getLeftText()[0].equalsIgnoreCase("Captain") || lowerThird.getLeftText()[0].equalsIgnoreCase("Captain Wicket-Keeper") ||
		    						...
		    						foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata, lowerThird.getLeftText()[0].toUpperCase(), 
		    								"", null, 0, foreignLanguageDataList);
		    						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_NameSuper_Center$TopLine$img_Text1$Side" + whichSide + "$English$txt_Designation*GEOM*TEXT SET ", 
		    								config, Constants.BCCI, print_writers, foreignLanguageData);
	                ...
	                case "l":
	    					...
	    					for(int iStat = 0; iStat < lowerThird.getTitlesText().length; iStat++) {
	    						...
	    						foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata, lowerThird.getTitlesText()[iStat], 
	    								"", null, 0, foreignLanguageDataList);
	    						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$SublineGrp$Side" + whichSide 
	    								+ "$select_Subline$1$Title$" + (iStat + 1) + "$English$txt_1*GEOM*TEXT SET ",config, Constants.BCCI, print_writers, foreignLanguageData);
	    				...
	    				case Constants.TRI_SERIES:  case Constants.MT20: case Constants.TG20: case Constants.APLT20:
		    				...
		    				case Constants.TG20: case Constants.APLT20:
		    					for(int iStat = 0; iStat < lowerThird.getTitlesText().length; iStat++) {
		    						...
		    						foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata, lowerThird.getTitlesText()[iStat].trim(), 
		    								"", null, 0, foreignLanguageDataList);
		    						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$SublineGrp$Side" + whichSide 
		    								+ "$select_Subline$1$Title$" + (iStat + 1) + "$English$txt_1*GEOM*TEXT SET ",config, Constants.BCCI, print_writers, foreignLanguageData);
	    				...
	    				case Constants.TRI_SERIES:  case Constants.MT20: case Constants.TG20: case Constants.APLT20:
		    				...
		    				case Constants.TG20: case Constants.APLT20:
		    					for(int iStat = 0; iStat < lowerThird.getTitlesText().length; iStat++) {
		    						...
		    						foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata, lowerThird.getTitlesText()[iStat].trim().toUpperCase(), 
		    								"", null, 0, foreignLanguageDataList);
		    						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$SublineGrp$Side" + whichSide 
		    								+ "$select_Subline$1$Title$" + (iStat + 1) + "$English$txt_1*GEOM*TEXT SET ",config, Constants.BCCI, print_writers, foreignLanguageData);
	    			...
	    			case "u": case "Shift_F3":
	    				...
	    				case Constants.TRI_SERIES:  case Constants.MT20: case Constants.TG20: case Constants.APLT20:
		    				...
		    				case Constants.TG20: case Constants.APLT20:
		    					...
		    					for(int i=0; i<10; i++) {
		    						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$SublineGrp$Side" + whichSide 
			    							+ "$select_Subline$1$Title$" + (i+1) + "$English$txt_1*GEOM*TEXT SET \0", print_writers);
		    						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$SublineGrp$Side" + whichSide 
			    							+ "$select_Subline$1$Title$" + (i+1) + "$Telugu$txt_1*GEOM*TEXT SET \0", print_writers);
		    					}
		    					...
		    					for(int iStat = 0; iStat < lowerThird.getTitlesText().length; iStat++) {
		    						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$SublineGrp$Side" + whichSide 
			    							+ "$select_Subline$1$Title$" + (iStat + 1) + "$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
		    						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$SublineGrp$Side" + whichSide 
		    								+ "$select_Subline$1$Title$" + (iStat + 1) + "$English$txt_1*GEOM*TEXT SET " + lowerThird.getTitlesText()[iStat] + "\0", print_writers);
		    						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$SublineGrp$Side" + whichSide 
			    							+ "$select_Subline$1$Title$" + (iStat + 1) + "$Telugu$txt_1*GEOM*TEXT SET " + lowerThird.getTitlesText()[iStat] + "\0", print_writers);
...
//		    					}
		    					...
		    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$SublineGrp$Side" + whichSide 
		    							+ "$select_Subline$1$Left$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
		    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$SublineGrp$Side" + whichSide 
		    							+ "$select_Subline$2$Left$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
...
//	    					}
	    			...
	    			case "Alt_Shift_F12":
    					...
    					for(int i=0; i<lowerThird.getTitlesText().length; i++) {
    						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$SublineGrp$Side" + whichSide 
	    							+ "$select_Subline$1$Title$" + (i + 1) + "$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
    						
    						foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata, lowerThird.getTitlesText()[i], 
    								"", null, 0, foreignLanguageDataList);
    						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$SublineGrp$Side" + whichSide 
    								+ "$select_Subline$1$Title$" + (i + 1) + "$English$txt_1*GEOM*TEXT SET ",config, Constants.BCCI, print_writers, foreignLanguageData);
    					}
	    				...
	    				case Constants.TRI_SERIES:  case Constants.MT20: case Constants.TG20: case Constants.APLT20:
		    				...
		    				case Constants.TG20: case Constants.APLT20:
    							...
    							CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide 
		    							+ "$2$Left$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
    							CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide 
		    							+ "$3$Left$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
	    						...
	    						foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.TEAM, CricketUtil.TEAMNAME_3, multilanguagedata, inning.getBatting_team().getTeamName1(), 
	    								"", null, 0, foreignLanguageDataList);
	    						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide 
	    								+ "$select_Subline$2$Left$English$txt_1*GEOM*TEXT SET ",config, Constants.BCCI, print_writers, foreignLanguageData);
	    						
	    						foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.TEAM, CricketUtil.TEAMNAME_3, multilanguagedata, inning2.getBatting_team().getTeamName1(), 
	    								"", null, 0, foreignLanguageDataList);
	    						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Position_With_Graphics$SublineGrp$Rows$Side" + whichSide 
	    								+ "$select_Subline$3$Left$English$txt_1*GEOM*TEXT SET ",config, Constants.BCCI, print_writers, foreignLanguageData);
	    						...
	    						for(int i = 0; i < lowerThird.getTitlesText().length; i++) {
		    					    ...
		    					    CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$SublineGrp$Side" + whichSide 
			    							+ "$select_Subline$1$Title$" + (i + 1) + "$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
		    					    foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata, title, 
		    								"", null, 0, foreignLanguageDataList);
		    					    CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$SublineGrp$Side" + whichSide 
		    								+ "$select_Subline$1$Title$" + (i + 1) + "$English$txt_1*GEOM*TEXT SET ",config, Constants.BCCI, print_writers, foreignLanguageData);
		    					}
	    			...
	    			case "Control_F3"://Comparison
	                   ...
	                   case Constants.TG20: case Constants.APLT20:
	                	   ...
	                	   CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$Lt_Comparison$Side0" + whichSide + "$DataPart$"
	                	   		+ "BottomPart$Text$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
//	                	   lowerThird.getBallsFacedText()
	                	   	String[] result = splitText("AFTER OVERS");
	                	   	
						List<String> insertText = new ArrayList<>();
	
						insertText.add(lowerThird.getBallsFacedText());
						foreignLanguageDataList = CricketFunctions.AssembleMultiLanguageData(CricketUtil.DICTIONARY, "", multilanguagedata,result[1],
							    "", insertText, 1,foreignLanguageDataList);
	
						foreignLanguageData.add(CricketFunctions.MergeForeignLanguageDataListToSingleObject(foreignLanguageDataList));
						
						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$Lt_Comparison$Side0" + whichSide + "$DataPart$"
								+ "BottomPart$Text$English$txt_SubTitle*GEOM*TEXT SET ",config, Constants.BCCI, print_writers, foreignLanguageData);
						
						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_Comparison$Side0" + whichSide + "$DataPart$" +
	    							"BottomPart$Text$English$txt_SubTitle*GEOM*TEXT SET AFTER " + lowerThird.getBallsFacedText() + " OVERS\0", print_writers);
	                	   break;
	                   }
					break;
	    		case "Control_Shift_O": 
...
//	  	    							"$Select*FUNCTION*Omo*vis_con SET 0\0", print_writers);
	  	    					...
	  	    					CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + whichSide + "$TopLine$img_Text1$" + row_id + 
	  	    							"$Pic$img_Base2*TEXTURE*IMAGE SET " + (lowerThird.getWhichTeamFlag().equalsIgnoreCase("LEGENDS") ? base1Path : base2Path) + lowerThird.getWhichTeamFlag() + "\0", print_writers);
		  	  	            	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + whichSide + "$TopLine$img_Text1$" + row_id + 
		  	  	            			"$Name_Grp$img_Text2*TEXTURE*IMAGE SET " + (lowerThird.getWhichTeamFlag().equalsIgnoreCase("LEGENDS") ? text1Path : text2Path)  + lowerThird.getWhichTeamFlag() + "\0", print_writers);
		  	  	            	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + whichSide + "$TopLine$img_Text1$" + row_id + 
		  	  	            			"$Captain$img_Text2*TEXTURE*IMAGE SET " + (lowerThird.getWhichTeamFlag().equalsIgnoreCase("LEGENDS") ? text1Path : text2Path)  + lowerThird.getWhichTeamFlag() + "\0", print_writers);
		  	  	            	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side"+ whichSide + "$TopLine$img_Text1$" + row_id + 
		  	  	            			"$Captain$Select$img_Text1*TEXTURE*IMAGE SET " + (lowerThird.getWhichTeamFlag().equalsIgnoreCase("LEGENDS") ? text1Path : text2Path)  + lowerThird.getWhichTeamFlag() + "\0", print_writers);
			    				...
			    				case Constants.TG20: case Constants.APLT20:
			    					...
			    					CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + whichSide + "$TopLine$img_Text1$" + row_id + 
			    							"$Name_Grp$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
			    					foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.PLAYER, CricketUtil.TICKERNAME, multilanguagedata, bc.getTicker_name(), 
		    								"", null, 0, foreignLanguageDataList);
		    						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + whichSide + "$TopLine$img_Text1$" + row_id + 
		    								"$Name_Grp$English$txt_LastName*GEOM*TEXT SET ",config, Constants.BCCI, print_writers, foreignLanguageData);
		    						...
		    						if(config.getPrimaryIpAddress().equalsIgnoreCase(Constants.LOCALHOST)) {
		    							CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + whichSide + "$TopLine$img_Text1$" + row_id + "$Pic$img*TEXTURE*IMAGE SET "
		  	    						...
		  	    						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + whichSide + "$TopLine$img_Text1$" + row_id + "$Pic$img*TEXTURE*IMAGE SET "
		  	    								+ "\\\\" + config.getPrimaryIpAddress() + "\\\\" + Constants.TG20_PHOTO_PATH + "\\\\" + team.getTeamName4() + "\\\\" + Constants.CENTER + "\\\\" + bc.getPhoto()
		  	    								+ CricketUtil.PNG_EXTENSION + "\0", print_writers);
		  	    					}
		    						break;
		    					}
			    				...
			    				case Constants.TG20: case Constants.APLT20:
			    					if(bc.getRole().equalsIgnoreCase(CricketUtil.BATSMAN) || bc.getRole().equalsIgnoreCase("BATTER") || 
		  	    						...
		  	    						if(bc.getBattingStyle().equalsIgnoreCase("RHB")) {
		  	    							CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side"+ whichSide + 
		  	    	    							"$TopLine$img_Text1$" + row_id + "$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "Bat_Icon" + "\0", print_writers);
		  	    						}else if(bc.getBattingStyle().equalsIgnoreCase("LHB")) {
		  	    							CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side"+ whichSide + 
		  	    	    							"$TopLine$img_Text1$" + row_id + "$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "Batsman_Lefthand" + "\0", print_writers);
		  	    						}
		  	    					}else if(bc.getRole().equalsIgnoreCase(CricketUtil.BOWLER)) {
		  	    						if(bc.getBowlingStyle() == null) {
		  	    							CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side"+ whichSide + 
		  	    	    							"$TopLine$img_Text1$" + row_id + "$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "Bowler" + "\0", print_writers);
		  	    						}else {
		  	    							switch (bc.getBowlingStyle().toUpperCase()) {
		  	    							case "RFM": case "RF": case "RMF": case "RM": case "RSM": case "LF": case "LFM": case "LMF": case "LM":
		  	    								CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side"+ whichSide + 
		  		    	    							"$TopLine$img_Text1$" + row_id + "$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "FastBowler" + "\0", print_writers);
		  	    								break;

		  	    							case "LSL": case "WSL": case "LCH": case "RLG": case "WSR": case "LSO":
		  	    								CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side"+ whichSide + 
		  		    	    							"$TopLine$img_Text1$" + row_id + "$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "SpinBowler" + "\0", print_writers);
		  	    								break;
		  	    							case "ROB":
		  	    								CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side"+ whichSide + 
		  		    	    							"$TopLine$img_Text1$" + row_id + "$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "Off_Spin" + "\0", print_writers);
		  	    								break;
		  	    							case "RLB":
		  	    								CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side"+ whichSide + 
		  		    	    							"$TopLine$img_Text1$" + row_id + "$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "Leg_Spin" + "\0", print_writers);
		  	    								break;	
		  	    							}
		  	    						}
		  	    					}else if(bc.getRole().equalsIgnoreCase(CricketUtil.ALL_ROUNDER)) {
		  	    						if(bc.getBowlingStyle() == null) {
		  	    							if(bc.getBattingStyle().equalsIgnoreCase("LHB")) {
		  	    								CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side"+ whichSide + 
		  		    	    							"$TopLine$img_Text1$" + row_id + "$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "Off_Spin_Allrounder_Left" + "\0", print_writers);
		  	    							}else {
		  	    								CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side"+ whichSide + 
		  		    	    							"$TopLine$img_Text1$" + row_id + "$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "FastBowlerAllrounder" + "\0", print_writers);
		  	    							}
		  	    							...
		  	    							case "RFM": case "RF": case "RMF": case "RM": case "RSM": case "LF": case "LFM": case "LMF": case "LM":
		  	    								if(bc.getBattingStyle().equalsIgnoreCase("LHB")) {
		  	    									CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side"+ whichSide + 
		  	    	    	    							"$TopLine$img_Text1$" + row_id + "$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "Pace_BowlerAllrounerLeftHand" + "\0", print_writers);
		  	    								}else {
		  	    									CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side"+ whichSide + 
		  	    	    	    							"$TopLine$img_Text1$" + row_id + "$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "FastBowlerAllrounder" + "\0", print_writers);
		  	    								}
		  	    							...
		  	    							case "ROB": case "RLB": case "LSL": case "WSL": case "LCH": case "RLG": case "WSR": case "LSO":
		  	    								if(bc.getBattingStyle().equalsIgnoreCase("LHB")) {
		  	    									...
		  	    									CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side"+ whichSide + 
		  	    	    	    							"$TopLine$img_Text1$" + row_id + "$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "Off_Spin_Bat" + "\0", print_writers);
		  	    								}
		  	    								break;
		  	    							}
		  	    						}
	  	    					...
	  	    					if(bc.getCaptainWicketKeeper() != null) {
	  	    						if (bc.getCaptainWicketKeeper().equalsIgnoreCase(CricketUtil.CAPTAIN)) {
	  				    				...
	  				    				case Constants.TG20:
	  				    					CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side"+ whichSide + 
		  	    	    							"$TopLine$img_Text1$" + row_id + "$Bottom$Select$Batsman_Lefthand*TEXTURE*IMAGE SET " + Constants.TG20_ICONS + "Keeper" + "\0", print_writers);
	  			    						break;
	  			    					}
	  	    						} else if (bc.getCaptainWicketKeeper().equalsIgnoreCase(CricketUtil.WICKET_KEEPER)) {
...
//	  	    							+ "*FUNCTION*Grid*num_col SET " + row_id + "\0", print_writers);
			    				...
			    				case Constants.TG20: case Constants.APLT20:
			    					CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + whichSide + "$TopLine$img_Text1$" + row_id + 
		  							...
		  							String impactType = CricketFunctions.checkBatAndBallImpactInOutPlayer(match.getEventFile().getEvents(), bc.getPlayerId());
		  							if (!impactType.isEmpty()) {
		  								switch(impactType) {
		  								case "IMP_IN": case "CON_IN":
		  									CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + whichSide + "$TopLine$img_Text1$" + row_id + 
					    							"$Impact$select_Impact*FUNCTION*Omo*vis_con SET 2\0", print_writers);
		  									CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + whichSide + "$TopLine$img_Text1$" + row_id + 
		  											"$Impact$ImpactAll$Impact$txt_Name*GEOM*TEXT SET " + impactType.replace("_IN", "") + "\0", print_writers);
		  									break;
		  								case "IMP_OUT": case "CON_OUT":
		  									CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + whichSide + "$TopLine$img_Text1$" + row_id + 
					    							"$Impact$select_Impact*FUNCTION*Omo*vis_con SET 1\0", print_writers);
		  									CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + whichSide + "$TopLine$img_Text1$" + row_id + 
		  											"$Impact$ImpactAll$Sub$txt_Name*GEOM*TEXT SET " + (impactType.equalsIgnoreCase("IMP_OUT") ? "SUB" : "CON") + "\0", print_writers);
		  									break;
		  								}
		  							}
			    					...
			    					foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.PLAYER, CricketUtil.TICKERNAME, multilanguagedata, bc.getPlayer().getTicker_name(), 
		    								"", null, 0, foreignLanguageDataList);
		    						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + whichSide 
		    								+ "$TopLine$img_Text1$" + row_id + "$Name_Grp$English$txt_LastName*GEOM*TEXT SET ",config, Constants.BCCI, print_writers, foreignLanguageData);
		    						...
		    						if(config.getPrimaryIpAddress().equalsIgnoreCase(Constants.LOCALHOST)) {
		    							CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + whichSide + "$TopLine$img_Text1$" + row_id + "$Pic$img*TEXTURE*IMAGE SET "
		  	    						...
		  	    						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + whichSide + "$TopLine$img_Text1$" + row_id + "$Pic$img*TEXTURE*IMAGE SET "
		  	    								+ Constants.TG20_LOCAL_PHOTO_PATH + "\\\\" + team.getTeamName4() + "\\\\" + Constants.CENTER + "\\\\" + bc.getPlayer().getPhoto() + CricketUtil.PNG_EXTENSION + "\0", print_writers);
		  	    					}else {
		  	    						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + whichSide + "$TopLine$img_Text1$" + row_id + "$Pic$img*TEXTURE*IMAGE SET "
		  	    								+ "\\\\" + config.getPrimaryIpAddress() + "\\\\" + Constants.TG20_PHOTO_PATH + "\\\\" + Constants.BLANK + CricketUtil.PNG_EXTENSION + "\0", print_writers);
		  	    						
		  	    						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_PlayingXI$Side" + whichSide + "$TopLine$img_Text1$" + row_id + "$Pic$img*TEXTURE*IMAGE SET "
		  	    								+ "\\\\" + config.getPrimaryIpAddress() + "\\\\" + Constants.TG20_PHOTO_PATH + "\\\\" + team.getTeamName4() + "\\\\" + Constants.CENTER + "\\\\" + bc.getPlayer().getPhoto()
		  	    								+ CricketUtil.PNG_EXTENSION + "\0", print_writers);
		  	    					}
		    					}
	    			  ...
	    			  case "Control_Shift_L":
						...
						CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + whichSide + "$DataPart$BottomPart"
								+ "$Text$Side" + whichSide +"$English$txt_SubTitle*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
	    					  break;
	    				  default:
	    					  CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$Lt_MatchID$Side0" + whichSide + "$DataPart$BottomPart" + 
	    			  ...
	    			  case "Control_Shift_B":
	  	    			...
	  	    			case Constants.TRI_SERIES:  case Constants.MT20:  case Constants.TG20: case Constants.APLT20:
	  	    				...
	  	    				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_NextToBat$Side"+ whichSide +"$TopLine$img_Text1$1$Impact*ACTIVE SET 0\0", print_writers);
	  	    				CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_NextToBat$Side"+ whichSide +"$TopLine$img_Text1$2$Impact*ACTIVE SET 0\0", print_writers);
	  	    				...
	  	    				for(BattingCard bc : inning.getBattingCard()) {
	  	    					...
	  	    					case CricketUtil.STILL_TO_BAT:
	  	    						...
	  	    						case Constants.TG20: case Constants.APLT20:
	  	    							if(config.getPrimaryIpAddress().equalsIgnoreCase(Constants.LOCALHOST)) {
	  	    								CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_NextToBat$Side"+ whichSide +"$TopLine$img_Text1$"+ rowId + "$Pic$img"
	  	    										+ "*TEXTURE*IMAGE SET " + Constants.TG20_LOCAL_PHOTO_PATH + Constants.BLANK + CricketUtil.PNG_EXTENSION + " \0", print_writers);
	  	    								
	  	    								CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_NextToBat$Side"+ whichSide +"$TopLine$img_Text1$"+ rowId + "$Pic$img"
	  	    										+ "*TEXTURE*IMAGE SET " + Constants.TG20_LOCAL_PHOTO_PATH + "\\\\" + inning.getBatting_team().getTeamName4() + "\\\\" + Constants.CENTER 
	  	    										+ "\\\\" + bc.getPlayer().getPhoto() + CricketUtil.PNG_EXTENSION + " \0", print_writers);
	  	    							}else {
	  	    								CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_NextToBat$Side"+whichSide+"$TopLine$img_Text1$"+ rowId + "$Pic$img"
	  	    										+ "*TEXTURE*IMAGE SET " + "\\\\" + config.getPrimaryIpAddress() + "\\\\" + Constants.TG20_PHOTO_PATH + Constants.BLANK 
	  	    										+ CricketUtil.PNG_EXTENSION + " \0", print_writers);
	  	    								
	  	    								CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_NextToBat$Side"+whichSide+"$TopLine$img_Text1$"+ rowId + "$Pic$img"
	  	    										+ "*TEXTURE*IMAGE SET " + "\\\\" + config.getPrimaryIpAddress() + "\\\\" + Constants.TG20_PHOTO_PATH + "\\\\" + inning.getBatting_team().getTeamName4() 
	  	    										+"\\\\" + Constants.CENTER +"\\\\"+bc.getPlayer().getPhoto()+CricketUtil.PNG_EXTENSION + " \0", print_writers);
	  	    							}
	  	    							break;
	  	    						}
	  	    						
	  	    						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_NextToBat$Side" + whichSide + "$TopLine$img_Text1$" + rowId + "$Pic$img_Base2*TEXTURE*IMAGE SET " 
	  	    								+ (lowerThird.getWhichTeamFlag().equalsIgnoreCase("LEGENDS") ? base1Path : base2Path) + lowerThird.getWhichTeamFlag() + "\0", print_writers);
			  	  	            	CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_NextToBat$Side" + whichSide + "$TopLine$img_Text1$" + rowId + "$Name_Grp$img_Text2*TEXTURE*IMAGE SET " 
			  	  	            			+ (lowerThird.getWhichTeamFlag().equalsIgnoreCase("LEGENDS") ? text1Path : text2Path) + lowerThird.getWhichTeamFlag() + "\0", print_writers);
			  	  	            	
	  	    						CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$LT_NextToBat$Side"+ whichSide +"$TopLine$img_Text1$"+ rowId + "$Bottom$Select"
	  	    								+ "*FUNCTION*Omo*vis_con SET 1\0", print_writers);
	  	    						
	  	    						switch (config.getBroadcaster().toUpperCase()) {
	  	    						case Constants.TG20: case Constants.APLT20:
	  	    							CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$LT_NextToBat$Side" + whichSide + "$TopLine$img_Text1$" + rowId + 
		  	    								"$Name_Grp$select_Language*FUNCTION*Omo*vis_con SET ", config, Constants.TG20, print_writers, foreignLanguageOmo);
		  								foreignLanguageData = CricketFunctions.AssembleMultiLanguageData(CricketUtil.PLAYER, CricketUtil.TICKERNAME, multilanguagedata, bc.getPlayer().getTicker_name(), 
		  										"", null, 0, foreignLanguageDataList);
		  								CricketFunctions.DoadWriteVariousLanguageTextToEachViz("RENDERER*FRONT_LAYER*TREE*$LT_NextToBat$Side" + whichSide + "$TopLine$img_Text1$" + rowId + "$Name_Grp$English"
		  										+ "$txt_LastName*GEOM*TEXT SET ", config, Constants.TG20, print_writers, foreignLanguageData);
	  	    							break;
	  	    						default:
			...
			for(int iSubLine = 1; iSubLine <= lowerThird.getNumberOfSubLines(); iSubLine++) {
				...
				if(lowerThird.getTitlesText() != null) {
					...
					case Constants.TG20: case Constants.APLT20:
						for(int iTitle = 1; iTitle <= lowerThird.getTitlesText().length; iTitle++) {
							CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Overall_Position_Y_In_Out$SublineGrp$Side" + WhichSide +
									"$select_Subline$"+ iSubLine + "$Title$" + iTitle + "*ACTIVE SET 1\0", print_writers);
//							CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Overall_Position_Y_In_Out$SublineGrp$Side" + WhichSide +
//								"$select_Subline$"+ iSubLine + "$Title$" + iTitle + "$English$txt_1*ACTIVE SET 1 \0", print_writers);
						...
						for(int iTitle = lowerThird.getTitlesText().length + 1; iTitle <= 10; iTitle++) {
							CricketFunctions.DoadWriteCommandToAllViz("-1 RENDERER*FRONT_LAYER*TREE*$gfx_LowerThird$Overall_Position_Y_In_Out$SublineGrp$Side" + WhichSide +
									"$select_Subline$"+ iSubLine + "$Title$" + iTitle + "*ACTIVE SET 0\0", print_writers);
						}
						break;	
					}
...
//						BottomTitle[i-4] = griff.get(i).getOpponentTeam().getTeamName4() + " - " + this_data_str.get(i).toUpperCase() + " T20I";
	...
	public void Sponsor(String IsSponsor,int WhichSide) {
		switch(IsSponsor.toUpperCase()){
		case "YES":
...
//		System.out.println(result);
	...
	public static String[] splitText(String text) {
	    Matcher m = Pattern.compile("(?i)^.*?\\bARE\\b\\s+(\\d+)\\s+(.*)$")
	                       .matcher(text);
